...
type UserSpinWheelDataResult struct {
	...
	RewardValue   int                      `json:"reward_value"` //中奖值,会随机浮动
	Status        data.SpinWheelUserStatus `json:"status"`       //当前状态
	Count         int                      `json:"count"`        //已抽奖次数
	ResetTime     int64                    `json:"reset_time"`
	TicketsNeeded []int                    `json:"tickets_needed"` //抽奖需要的票数
	TicketPrice   int                      `json:"ticket_price"`   //票的价格
}

func toUserSpinWheelDataResult(userData *data.SpinWheelUserData, spinWheel *data.SpinWheelData) *UserSpinWheelDataResult {
	return &UserSpinWheelDataResult{
		UserId:        userData.UserId,
		GameId:        spinWheel.GameId,
		SpinWheelId:   spinWheel.Id,
		Rewards:       spinWheel.Rewards,
		RewardIndex:   userData.RewardIndex,
		RewardValue:   userData.RewardValue,
		Status:        userData.Status,
		Count:         userData.Count,
		ResetTime:     utils.GetNextResetTime().UnixMilli(),
		TicketsNeeded: config.GetConfig().SpinWheelCfg.TicketsNeeded,
		TicketPrice:   config.GetConfig().SpinWheelCfg.TicketPrice,
	}
}
...
// @Authorize	"tma {token}"
// @Success	200	{object}	api.JSONResult{data=data.UserSpinWheelDataResult}	"用户轮盘数据"
...
func apiUserSpinWheel(c *gin.Context, user *data.DashFunUser) {
	...
	//	return
	//}
	spinData := spinwheelcenter.Get().GetDashFunSpinWheelData()
	result, err := spinwheelcenter.Get().UserSpinWheel(user.Id, "")
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, RError(err.Error()))
		return
	}

	c.JSON(http.StatusOK, RSuccess(toUserSpinWheelDataResult(result, spinData)))
}
...
func apiUserGetSpinWheelInfo(c *gin.Context, user *data.DashFunUser) {
	...
	gameWheel := center.GetDashFunSpinWheelData()
	//if err != nil {
	//	c.AbortWithStatusJSON(http.StatusInternalServerError, RError(err.Error()))
	//	return
	//}
	...
	c.JSON(http.StatusOK, RSuccess(toUserSpinWheelDataResult(userData, gameWheel)))
}
...
func apiUserClaimSpinWheelReward(c *gin.Context, user *data.DashFunUser) {
	...
	center := spinwheelcenter.Get()
	gameWheel := center.GetDashFunSpinWheelData()
	userData, err := center.UserClaimReward(user.Id, "")
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, RError(err.Error()))
		return
	}
