...
import (
	...
	"dashfun_gamecenter/fishingverse/fvdata"
...
const leaderboardKey = "fishing_fp_leaderboard"
const leaderboardLoadKey = "fishing_fp_leaderboard_load"
const LeaderboardMinCount = 1234

var onceLeaderboardCenter sync.Once
var instLeaderboardCenter *FishingLeaderboardCenter
...
type FishingLeaderboardCenter struct {
	sync.RWMutex
	userCache map[string]*data.DashFunUser  //user缓存
	loading   bool                          //是否正在从数据库中读取数据建立排行榜中
	bots      map[string]*fvdata.FishingBot //所有的机器人
}

func Get() *FishingLeaderboardCenter {
	onceLeaderboardCenter.Do(func() {
		instLeaderboardCenter = &FishingLeaderboardCenter{}
		instLeaderboardCenter.init()
	})
	return instLeaderboardCenter
}

func (l *FishingLeaderboardCenter) init() {
...
func (l *FishingLeaderboardCenter) loadAllBots() {
	//从数据库中读取bot
	l.bots = make(map[string]*fvdata.FishingBot)

	botDao := dao.GetFishingLeaderboardBotDao()
	botData, err := botDao.LoadAllBots()
	if err != nil {
		log.Fatalf("loadAllBots err: %v", err)
	}

	bots := make([]*fvdata.FishingBot, 0)

	for _, b := range botData {
		bot := &fvdata.FishingBot{
			Data: b,
		}
...
func (l *FishingLeaderboardCenter) botBehaviour() {
...
func (l *FishingLeaderboardCenter) UploadScore(bot *data.FishingBotData) int64 {
	//存入redis
	rdb := rediscenter.Get()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()
...
func (l *FishingLeaderboardCenter) needLoad() bool {
	rdb := rediscenter.Get()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()
...
func (l *FishingLeaderboardCenter) initRedis() {
	l.loading = true
	coin, err := dao.GetCoinDao().FindCoinByName("FishingPoint")
	if err != nil {
...
func (l *FishingLeaderboardCenter) fillLeaderboard() {
	rdb := rediscenter.Get()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()
...
func (l *FishingLeaderboardCenter) onUserCoinChanged(event events.UserCoinChangedEvent) {
	if event.Coin.Name == "FishingPoint" {
		l.updateUserCoinAmount(event.UserId, event.UserData.Amount)
	}
}

func (l *FishingLeaderboardCenter) updateUserCoinAmount(userId string, amount int32) {
	if amount <= 0 {
		return
	}
	rdb := rediscenter.Get()
...
func (l *FishingLeaderboardCenter) onUserLogin(user *data.OnlineUser) {
	l.Lock()
	defer l.Unlock()
...
func (l *FishingLeaderboardCenter) GetUserRankAndScore(userId string) (int64, int64, error) {
	if l.loading {
		return 0, 0, nil
	}
	rdb := rediscenter.Get()
...
func (l *FishingLeaderboardCenter) GetTop(count int) ([]*FishingLeaderboard, error) {
	if l.loading {
		return make([]*FishingLeaderboard, 0), nil
	}
	rdb := rediscenter.Get()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()
	...
	for i, z := range r {
		...
		top = append(top, &FishingLeaderboard{
			Id:          uid,
			Rank:        int64(i + 1),
			Score:       int64(z.Score),
...
func (l *FishingLeaderboardCenter) getUser(id string) (*data.DashFunUser, error) {
	l.Lock()
	defer l.Unlock()
	u, ok := l.userCache[id]
